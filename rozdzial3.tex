...
\begin{algorithmic}[1]
	\Function{$F$}{$z,P[]$}	
		\Switch{$z$}
			...
			\EndCase
			\Case{K-e\_S}
				\State \textbf{return} $ 1 + P[\textit{K-a}] + 0.04 $
			\EndCase
			\Case{K-e\_P} 
				\If {$ DK == $ K-a}  
			...
			\EndCase
			\Case{Krl\_Z}
				\State \textbf{return} $ 1 + P[\textit{K-a}] + P[Hra] + 0.03 $
			\EndCase
			\Case{Hra\_K-e\_Kr}
...
\begin{algorithmic}[1]
	\Function{$R$}{$L[Y_n]$}
		...
		\ForAll {$z_i \in RK$}	\Comment Sprawdzenie szansy porażki, $i=|RK|$
			\Switch{$z_i$}
				\Case{Str\_DK}
					\State $P_{porazki}[z_i] \gets F_{max}(z_i) - 1.08$ \Comment Wynika z oceny zagrania Strazniczki przez algorytm zachłanny
				\EndCase
				\Case{Bar}
...
\begin{algorithmic}[1]
	\Function{$d_{mcts}$}{$I_n, X_n$}
		...
		\ForAll{ $z_i \in X_n$ } \Comment $i=|X_n|$
			\State $T \gets$ dodajDziecko($Nast(T, z_i)$)
		\EndFor
		\Repeat
